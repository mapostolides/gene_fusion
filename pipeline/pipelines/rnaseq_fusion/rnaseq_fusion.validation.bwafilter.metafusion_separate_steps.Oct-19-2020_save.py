...
from bfx import fusionannotator_db_hits 
...
#        return jobs
    ...
    def reannotate_cff_fusion(self):
        ...
        reann_job = merge_and_reannotate_cff_fusion.reannotate_cff_fusion(out_dir)
...
##END fusion reads capture pipeline
    ...
    def custom_filters(self):
        ...
        adjacent_noncoding_filter_job= custom_filters.filter_adjacent_noncoding(cff_dir) 
        cis_sage_job= custom_filters.output_cis_sage_file( cff_dir)
        ...
        filter_job = concat_jobs([
           custom_filter_job,
           adjacent_noncoding_filter_job,
           cis_sage_job
        ], name="custom_filters")
        jobs.append(filter_job)
        return jobs
  
    def fusionannotator_db_hits(self):
        """
        """
        jobs = []
        outdir = os.path.join("fusions", "cff")
        cluster=os.path.join(outdir, "merged.cff.renamed.reann.cluster.blck_filter.RT_filter.callerfilter2.ANC_filter")
        #JOBS
        db_hits_job = fusionannotator_db_hits.fusionannotator_db_hits(outdir, cluster)
        #CONCAT
        job = concat_jobs([
           db_hits_job 
        ], name="db_hits_job")
    ...
    def validate_fusions(self):
        ...
        """
        if self.args.valfile is None: 
            raise Exception('validate_fusions step requires --valfile argument')
        outdir = os.path.join("fusions", "cff")
