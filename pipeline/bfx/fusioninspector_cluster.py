...
def make_fusion_list(cluster_file, sample, out_dir, config_file=None, ini_section='fusioninspector'):
        fusion_list_file = os.path.join(out_dir, sample + ".fusion_list.txt")
        ...
        [fusion_list_file],
        command="""\
  {fusiontools_dir}/generate_fusioninspector_fusions_list_cluster.py \\
  {cluster_file} {sample} > {fusion_list_file}""".format(
...
# python $scripts_path/filter_fusions_using_fusioninspector_output.py $FI_output_file $renamed_cff_file $FI_validated_cff_file $FI_unvalidated_cff_file $FI_summary_file $caller_name
    return Job(
        ...
        [filtered_cff_file],
        command="""\
{fusiontools_dir}/filter_fusions_using_fusioninspector_output_cluster.py \\
  {FI_output_file} \\
  {renamed_cff_file} \\
